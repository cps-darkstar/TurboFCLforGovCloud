...
from app.core.config import TEST_USER_CONFIGS, settings
from app.models.turbofcl import FCLApplication, SAMDataCache, User
...
def get_test_password():
    ...
    try:
        response = ssm_client.get_parameter(
            Name="/turbofcl/test-user-password", WithDecryption=True
        )
    ...
    try:
        for email, config in TEST_USER_CONFIGS.items():
            ...
            if "application_data" in config and user.role == "FSO":
                existing_app = (
                    db.query(FCLApplication)
                    .filter(FCLApplication.user_id == user.id)
                    .first()
                )
...
async def main():
    ...
    for email, config in TEST_USER_CONFIGS.items():
        create_cognito_user(
            email=email, password=password, attributes=config["cognito_attributes"]
        )
