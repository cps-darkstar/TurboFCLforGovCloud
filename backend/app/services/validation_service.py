...
class ValidationService:
    ...
    async def validate_application(
        ...
        if sam_data and application.entity_type:
            issues.extend(
                self._validate_entity_type_match(application.entity_type, sam_data)
            )
    ...
    def _validate_entity_type_match(
        self, selected_type: EntityType, sam_data: SAMData
    ...
    def _validate_foci_status(
        self, foci_status: List[FOCIStatus]
    ) -> List[ValidationIssue]:
    ...
    def _assess_foci_impact(self, foci_status: List[FOCIStatus]) -> List[AIInsight]:
        ...
        if any(
            status in [FOCIStatus.FOREIGN_INVESTORS, FOCIStatus.FOREIGN_OWNERSHIP]
    ...
    def _validate_kmp_requirements(
        self, entity_type: EntityType, kmp_count: int
    ) -> Tuple[List[ValidationIssue], List[AIInsight]]:
    ...
    def _validate_document_requirements(
        self, entity_type: EntityType, uploaded_documents: List[str]
    ) -> Tuple[List[ValidationIssue], List[AIInsight]]:
        ...
        if missing_docs:
            completion_rate = (len(required_docs) - len(missing_docs)) / len(
                required_docs
            )
    ...
    def _validate_sam_status(self, sam_data: SAMData) -> List[ValidationIssue]:
        ...
        try:
            last_updated = datetime.fromisoformat(
                sam_data.last_updated.replace("Z", "+00:00")
            )
