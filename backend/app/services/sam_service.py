...
class SAMService:
                ...
                if response.status_code == 200:
                    ...
                    if "entityData" in data and len(data["entityData"]) > 0:
                        ...
                        sam_data = {
                            ...
                            "uei": entity.get("entityRegistration", {}).get("ueiSAM", uei),
                            "cage_code": entity.get("entityRegistration", {}).get("cageCode"),
                            "entity_structure": entity.get("entityRegistration", {}).get("entityStructureDesc", ""),
                            "state_of_incorporation": entity.get("entityRegistration", {}).get("stateOfIncorporation"),
                            "registration_status": entity.get("entityRegistration", {}).get("registrationStatus", ""),
                            "last_updated": entity.get("entityRegistration", {}).get("lastUpdateDate", datetime.now().isoformat())
                        }
    ...
    async def get_entity_structure_mapping(self, sam_entity_structure: str) -> Optional[str]:
    ...
    async def check_registration_status(self, sam_data: Dict[str, Any]) -> Dict[str, Any]:
