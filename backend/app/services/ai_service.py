...
class AIService:
    ...
    async def generate_chat_response(self, user_message: str, context: List[Dict] = None) -> Dict[str, Any]:
        """
...
Please provide a helpful, accurate answer based on the context above. If the context doesn't contain enough information, say so clearly and suggest next steps."""
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                generated_text = result.get('generated_text', 'I apologize, but I encountered an issue generating a response.')
    ...
    async def extract_kmps_from_document(self, document_text: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for person in persons:
                ...
                for role, keywords in role_keywords.items():
                    if any(keyword.lower() in context_window.lower() for keyword in keywords):
    ...
    def _get_context_around_name(self, text: str, name: str, window_size: int = 100) -> str:
    ...
    def _classify_document_type(self, text: str, filename: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ["articles of incorporation", "certificate of incorporation"]):
            return "Articles of Incorporation"
        elif any(keyword in text_lower for keyword in ["bylaws", "by-laws"]):
            return "Bylaws"
        elif any(keyword in text_lower for keyword in ["operating agreement", "llc agreement"]):
            ...
            return "Stock Ledger"
        elif any(keyword in filename_lower for keyword in ["license", "business license"]):
            return "Business License"
        elif any(keyword in text_lower for keyword in ["organization chart", "org chart"]):
            return "Organization Chart"
        elif any(keyword in text_lower for keyword in ["meeting minutes", "board minutes"]):
    ...
    async def validate_document_completeness(self, entity_type: str, uploaded_documents: List[str]) -> Dict[str, Any]:
        """
        ...
        completion_rate = (len(required_docs) - len(missing_docs)) / len(required_docs) if required_docs else 1.0
